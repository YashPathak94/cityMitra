...
// --- Social content agent output (daily generated posts) ---
// Supabase table `social_posts` (id text primary key, topic text, source
// text, content jsonb, created_at timestamptz) or the local JSON fallback.

import type { SocialPost } from "@/lib/social-agent";

const socialPostsFile = path.join(storageDir, "social-posts.json");
...
export async function saveSocialPost(post: SocialPost) {
  ...
  await mkdir(storageDir, { recursive: true });
  const all = await readJsonFile<SocialPost>(socialPostsFile);
  const next = all.filter((item) => item.id !== post.id);
  next.push(post);
  await writeFile(socialPostsFile, JSON.stringify(next.slice(-400), null, 2));
}

export async function listSocialPosts(limit = 30): Promise<SocialPost[]> {
  const client = supabase();
  ...
  if (client) {
    ...
    return (data || []).map((row) => ({
      ...
      source: (row.source as SocialPost["source"]) || "template",
