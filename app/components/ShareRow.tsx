"use client";

import { Check, Link2, Share2 } from "lucide-react";
import { useState } from "react";
import { trackActivity } from "@/lib/tracking";

function WhatsAppGlyph({ size = 15 }: { size?: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
      <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.297-.347.446-.52.149-.174.198-.298.297-.497.1-.198.05-.371-.025-.52-.074-.149-.668-1.612-.916-2.207-.241-.579-.486-.5-.668-.51-.173-.008-.372-.01-.57-.01-.198 0-.52.074-.793.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
    </svg>
  );
}

function XGlyph({ size = 13 }: { size?: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
      <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
    </svg>
  );
}

// Share row for guides/posts: WhatsApp first (it's where Indian city content
// actually travels), then X, native share on devices that support it, and a
// copy-link fallback. URLs resolve at click time so the row needs no props
// beyond a share title, and every tap is tracked as a "share" event.
export default function ShareRow({ title }: { title: string }) {
  const [copied, setCopied] = useState(false);

  function currentUrl() {
    return window.location.href.split("#")[0];
  }

  function track(channel: string) {
    trackActivity({ type: "share", label: `${channel}:${window.location.pathname}`.slice(0, 160) });
  }

  function shareWhatsApp() {
    track("whatsapp");
    window.open(`https://wa.me/?text=${encodeURIComponent(`${title} — ${currentUrl()}`)}`, "_blank", "noopener,noreferrer");
  }

  function shareX() {
    track("x");
    window.open(
      `https://twitter.com/intent/tweet?text=${encodeURIComponent(title)}&url=${encodeURIComponent(currentUrl())}`,
      "_blank",
      "noopener,noreferrer"
    );
  }

  async function shareNative() {
    if (typeof navigator.share !== "function") {
      await copyLink();
      return;
    }

    track("native");
    try {
      await navigator.share({ title, url: currentUrl() });
    } catch {
      // user dismissed the sheet — nothing to do
    }
  }

  async function copyLink() {
    track("copy");
    try {
      await navigator.clipboard.writeText(currentUrl());
      setCopied(true);
      window.setTimeout(() => setCopied(false), 2000);
    } catch {
      // clipboard unavailable — leave the button as-is
    }
  }

  return (
    <div className="shareRow" aria-label="Share this page">
      <span className="shareRowLabel">Share</span>
      <button type="button" className="shareBtn shareBtnWhatsApp" onClick={shareWhatsApp} aria-label="Share on WhatsApp">
        <WhatsAppGlyph /> WhatsApp
      </button>
      <button type="button" className="shareBtn" onClick={shareX} aria-label="Share on X">
        <XGlyph /> Post
      </button>
      <button type="button" className="shareBtn" onClick={shareNative} aria-label="Share via device">
        <Share2 size={14} /> Share
      </button>
      <button type="button" className="shareBtn" onClick={copyLink} aria-label="Copy link">
        {copied ? <Check size={14} /> : <Link2 size={14} />} {copied ? "Copied" : "Copy link"}
      </button>
    </div>
  );
}
